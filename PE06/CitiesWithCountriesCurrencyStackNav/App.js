...
function CountriesStackScreen({ countries, addCountry, updateCountry }) {
  return (
    <Stack.Navigator
      screenOptions={{
        headerStyle: {
    ...
    >
      ...
      <Stack.Screen
        name="Country"
        children={(props) => (
          <Country {...props} updateCountry={updateCountry} />
        )}
      />
    </Stack.Navigator>
  );
...
export default class App extends Component {
  ...
  updateCountry = (updatedCountry) => {
    const index = this.state.countries.findIndex(c => c.name === updatedCountry.name);
    const countries = [
